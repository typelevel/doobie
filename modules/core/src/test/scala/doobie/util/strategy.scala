...
object strategyspec extends H2Spec {
  ...
  def instrumented(i: KleisliInterpreter[IO]) =
    Transactor.interpret.set(xa, i.ConnectionInterpreter)

  "Connection configuration and safety" >> {

    "Connection.autoCommit should be set to false" in {
      val i = new Interp
      sql"select 1".query[Int].unique.transact(instrumented(i)).unsafeRunSync
    ...
    "Connection.commit should NOT be called on failure" in {
      val i = new Interp
      sql"abc".query[Int].unique.transact(instrumented(i)).attempt.unsafeRunSync.toOption must_== None
  ...
  "Connection configuration and safety (streaming)" >> {
    ...
    "Connection.autoCommit should be set to false" in {
      val i = new Interp
      sql"select 1".query[Int].stream.compile.toList.transact(instrumented(i)).unsafeRunSync
      i.Connection.autoCommit must_== Some(false)
    }
    ...
    "Connection.commit should NOT be called on failure" in {
      val i = new Interp
      sql"abc".query[Int].stream.compile.toList.transact(instrumented(i)).attempt.unsafeRunSync.toOption must_== None
      i.Connection.commit must_== None
    }
  ...
  "PreparedStatement safety" >> {
    ...
    "PreparedStatement.close should be called on failure" in {
      val i = new Interp
      sql"select 'x'".query[Int].unique.transact(instrumented(i)).attempt.unsafeRunSync.toOption must_== None
      i.PreparedStatement.close must_== Some(())
    }
  ...
  "PreparedStatement safety (streaming)" >> {
    ...
    "PreparedStatement.close should be called on failure" in {
      val i = new Interp
      sql"select 'x'".query[Int].stream.compile.toList.transact(instrumented(i)).attempt.unsafeRunSync.toOption must_== None
      i.PreparedStatement.close must_== Some(())
    }
