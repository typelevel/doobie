...
object fragments {
  ...
  /** Returns `f NOT IN ((fs0-A, fs0-B), (fs1-A, fs1-B), ...).`. */
  def notIn[F[_]: Reducible, A: util.Put, B: util.Put](f: Fragment, fs: F[(A,B)]): Fragment =
    fs.toList.map { case (a, b) => fr0"($a,$b)" }.foldSmash1(f ++ fr0"NOT IN (", fr",", fr")")

  /** Returns `f NOT IN ((fs0-A, fs0-B, fs0-C), (fs1-A, fs1-B, fs1-C), ...)`. */
  def notIn[F[_]: Reducible, A: util.Put, B: util.Put, C: util.Put](f: Fragment, fs: F[(A, B, C)]): Fragment =
