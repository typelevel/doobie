...
trait KleisliInterpreter[M[_]] { outer =>
  ...
  trait NClobInterpreter extends NClobOp.Visitor[Kleisli[M, NClob, *]] {
    ...
    override def raw[A](message: => String, f: NClob => A): Kleisli[M, NClob, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, NClob, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, NClob, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, NClob, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, NClob, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, NClob, Unit] = outer.trace(message)
  ...
  trait BlobInterpreter extends BlobOp.Visitor[Kleisli[M, Blob, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: Blob => A): Kleisli[M, Blob, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, Blob, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, Blob, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, Blob, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, Blob, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, Blob, Unit] = outer.trace(message)
  ...
  trait ClobInterpreter extends ClobOp.Visitor[Kleisli[M, Clob, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: Clob => A): Kleisli[M, Clob, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, Clob, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, Clob, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, Clob, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, Clob, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, Clob, Unit] = outer.trace(message)
  ...
  trait DatabaseMetaDataInterpreter extends DatabaseMetaDataOp.Visitor[Kleisli[M, DatabaseMetaData, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: DatabaseMetaData => A): Kleisli[M, DatabaseMetaData, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, DatabaseMetaData, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, DatabaseMetaData, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, DatabaseMetaData, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, DatabaseMetaData, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, DatabaseMetaData, Unit] = outer.trace(message)
    ...
    override def supportsSharding = primitive(_.supportsSharding, "supportsSharding")
  ...
  trait DriverInterpreter extends DriverOp.Visitor[Kleisli[M, Driver, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: Driver => A): Kleisli[M, Driver, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, Driver, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, Driver, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, Driver, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, Driver, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, Driver, Unit] = outer.trace(message)
  ...
  trait RefInterpreter extends RefOp.Visitor[Kleisli[M, Ref, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: Ref => A): Kleisli[M, Ref, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, Ref, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, Ref, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, Ref, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, Ref, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, Ref, Unit] = outer.trace(message)
  ...
  trait SQLDataInterpreter extends SQLDataOp.Visitor[Kleisli[M, SQLData, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: SQLData => A): Kleisli[M, SQLData, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, SQLData, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, SQLData, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, SQLData, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, SQLData, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, SQLData, Unit] = outer.trace(message)
  ...
  trait SQLInputInterpreter extends SQLInputOp.Visitor[Kleisli[M, SQLInput, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: SQLInput => A): Kleisli[M, SQLInput, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, SQLInput, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, SQLInput, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, SQLInput, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, SQLInput, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, SQLInput, Unit] = outer.trace(message)
  ...
  trait SQLOutputInterpreter extends SQLOutputOp.Visitor[Kleisli[M, SQLOutput, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: SQLOutput => A): Kleisli[M, SQLOutput, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, SQLOutput, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, SQLOutput, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, SQLOutput, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, SQLOutput, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, SQLOutput, Unit] = outer.trace(message)
  ...
  trait ConnectionInterpreter extends ConnectionOp.Visitor[Kleisli[M, Connection, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: Connection => A): Kleisli[M, Connection, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, Connection, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, Connection, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, Connection, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, Connection, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, Connection, Unit] = outer.trace(message)

    // domain-specific operations are implemented in terms of `primitive`
    override def abort(a: Executor) = primitive(_.abort(a), "abort", a)
    override def beginRequest = primitive(_.beginRequest, "beginRequest")
    ...
    override def endRequest = primitive(_.endRequest, "endRequest")
    ...
    override def setShardingKey(a: ShardingKey) = primitive(_.setShardingKey(a), "setShardingKey", a)
    override def setShardingKey(a: ShardingKey, b: ShardingKey) = primitive(_.setShardingKey(a, b), "setShardingKey", a, b)
    override def setShardingKeyIfValid(a: ShardingKey, b: Int) = primitive(_.setShardingKeyIfValid(a, b), "setShardingKeyIfValid", a, b)
    override def setShardingKeyIfValid(a: ShardingKey, b: ShardingKey, c: Int) = primitive(_.setShardingKeyIfValid(a, b, c), "setShardingKeyIfValid", a, b, c)
  ...
  trait StatementInterpreter extends StatementOp.Visitor[Kleisli[M, Statement, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: Statement => A): Kleisli[M, Statement, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, Statement, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, Statement, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, Statement, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, Statement, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, Statement, Unit] = outer.trace(message)
    ...
    override def enquoteIdentifier(a: String, b: Boolean) = primitive(_.enquoteIdentifier(a, b), "enquoteIdentifier", a, b)
    override def enquoteLiteral(a: String) = primitive(_.enquoteLiteral(a), "enquoteLiteral", a)
    override def enquoteNCharLiteral(a: String) = primitive(_.enquoteNCharLiteral(a), "enquoteNCharLiteral", a)
    ...
    override def isSimpleIdentifier(a: String) = primitive(_.isSimpleIdentifier(a), "isSimpleIdentifier", a)
    override def isWrapperFor(a: Class[_]) = primitive(_.isWrapperFor(a), "isWrapperFor", a)
  ...
  trait PreparedStatementInterpreter extends PreparedStatementOp.Visitor[Kleisli[M, PreparedStatement, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: PreparedStatement => A): Kleisli[M, PreparedStatement, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, PreparedStatement, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, PreparedStatement, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, PreparedStatement, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, PreparedStatement, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, PreparedStatement, Unit] = outer.trace(message)
  ...
  trait CallableStatementInterpreter extends CallableStatementOp.Visitor[Kleisli[M, CallableStatement, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: CallableStatement => A): Kleisli[M, CallableStatement, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, CallableStatement, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, CallableStatement, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, CallableStatement, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, CallableStatement, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, CallableStatement, Unit] = outer.trace(message)
  ...
  trait ResultSetInterpreter extends ResultSetOp.Visitor[Kleisli[M, ResultSet, *]] {
    ...
    // common operations delegate to outer interpreter
    override def raw[A](message: => String, f: ResultSet => A): Kleisli[M, ResultSet, A] = outer.raw(message, f)
    ...
    // Logger
    def error(message: => String): Kleisli[M, ResultSet, Unit] = outer.error(message)
    def warn(message: => String): Kleisli[M, ResultSet, Unit] = outer.warn(message)
    def info(message: => String): Kleisli[M, ResultSet, Unit] = outer.info(message)
    def debug(message: => String): Kleisli[M, ResultSet, Unit] = outer.debug(message)
    def trace(message: => String): Kleisli[M, ResultSet, Unit] = outer.trace(message)
