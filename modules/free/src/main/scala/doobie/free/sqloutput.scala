...
object sqloutput { module =>
  ...
  object SQLOutputOp {
    ...
    final case class Log(level: LogLevel, throwable: Option[Throwable], message: () => String) extends SQLOutputOp[Unit] {
  ...
  // Logging primitives
  def error(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Error, None, () => message))
  def warn (message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Warn,  None, () => message))
  def info (message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Info,  None, () => message))
  def debug(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Debug, None, () => message))
  def trace(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Trace, None, () => message))

  def error(t: Throwable)(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Error, Some(t), () => message))
  def warn (t: Throwable)(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Warn,  Some(t), () => message))
  def info (t: Throwable)(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Info,  Some(t), () => message))
  def debug(t: Throwable)(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Debug, Some(t), () => message))
  def trace(t: Throwable)(message: => String) = FF.liftF[SQLOutputOp, Unit](Log(LogLevel.Trace, Some(t), () => message))
  ...
  // SQLOutputIO is a Logger
  implicit val LoggerSQLOutputIO: io.chrisdavenport.log4cats.Logger[SQLOutputIO] =
    new io.chrisdavenport.log4cats.Logger[SQLOutputIO] {
