...
class RangeSuite extends munit.FunSuite {
  ...
  test("decode for partially filled should return correct range") {
    ...
    assertEquals(decode[Double]("[,33.14)"), Right(Range[Double](None, Some(33.14D), `[_,_)`)))
    assertEquals(decode[Double]("[3.14,)"), Right(Range[Double](Some(3.14D), None, `[_,_)`)))

    assertEquals(decode[LocalDate]("[2024-01-01,infinity]"), Right(Range(dateLowerBound.toLocalDate, LocalDate.MAX, `[_,_]`)))
    assertEquals(decode[LocalDate]("[-infinity,2024-02-02]"), Right(Range(LocalDate.MIN, dateUpperBound.toLocalDate, `[_,_]`)))
  }
  ...
  test("encode for partially filled should return correct string value") {
    ...
    assertEquals(encode(Range[Double](None, Some(33.14D), `[_,_)`)), "[,33.14)")
    assertEquals(encode(Range[Double](Some(3.14D), None, `[_,_)`)), "[3.14,)")

    assertEquals(encode(Range(dateLowerBound.toLocalDate, LocalDate.MAX, `[_,_]`)), "[2024-01-01,infinity]")
    assertEquals(encode(Range(LocalDate.MIN, dateUpperBound.toLocalDate, `[_,_]`)), "[-infinity,2024-02-02]")
  }
