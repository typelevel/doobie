...
object resultset2 extends RWSFWorld with EventLogging with IndexedState {
  import rwsfops._
  ...
  class Result[I: Composite, O](p0: Process1[Ans[I], Ans[O]]) {
    import Process.{repeatEval, End}
    import Task.delay

    ////// IMPLEMENTATION DETAILS

    private def process(rs: ResultSet): Process[Task, Ans[I]] = {
    ...
    // the fundamental op?
    def pipe[O2](transform: Process1[Ans[O], Ans[O2]]): Result[I, O2] = 
      new Result(p0 pipe transform)

    ////// Transform

    def collect[O2](f: PartialFunction[O, O2]) =
      pipe(process1.collect {
        case (w, \/-(o)) if f.isDefinedAt(o) => (w, \/-(f(o)))
        case err @ (_, -\/(_)) => err // include errors
      })
    ...
    def dropWhile(f: O => Boolean): Result[I, O] =
      pipe(process1.dropWhile { 
        case (_, \/-(o)) => f(o)
        case _ => false // don't drop errors
      })

    def filter(f: O => Boolean): Result[I, O] =
      pipe(process1.filter { 
        case (_, \/-(o)) => f(o)
        case _           => true // include errors
      })
