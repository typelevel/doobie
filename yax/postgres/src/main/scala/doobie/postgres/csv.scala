...
import doobie.postgres.imports.{ PHC, PFCM }
...
object csv {
  ...
  trait CsvInstances extends CsvInstances0 { this: Csv.type =>
    ...
    implicit val booleanInstance: Csv[Boolean] =
      instance {
        case (true,  q, e) => _ append "TRUE"
        case (false, q, e) => _ append "FALSE"
      }
  ...
  implicit class CsvOps[A](a: A)(implicit ev: Csv[A]) {
    def csv = ev.encode(a, '"', '"')
  }

  implicit class PgFragmentOps(f: Fragment) {
    import java.io._
    def copyIn[F[_]: Foldable, A: Csv](fa: F[A]): ConnectionIO[Long] = {
      val data: Reader = new StringReader(fa.foldMap(s => s.csv + "\n"))
