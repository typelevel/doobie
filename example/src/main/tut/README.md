...
```scala
import scalaz._, Scalaz._, scalaz.concurrent.Task, scalaz.stream.io, doobie.imports._
...
val load = sql"RUNSCRIPT FROM 'world.sql' CHARSET 'UTF-8'".update.run.void
...
def q(e: Double) = sql"SELECT NAME, CONTINENT, POPULATION FROM COUNTRY WHERE LIFEEXPECTANCY >= $e".query[Country].process

// Our program is two transactions, one loading data and one querying and streaming results to stdout
val program = xa.transact(load) *> xa.transact(q(80)).take(5).map(_.toString).to(io.stdOutLines).run
